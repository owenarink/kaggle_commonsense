...
from src.models.bert_attention_experiments import BertRelSimpleTransformer
from src.tokenizer_bbpe import ensure_bbpe, load_bbpe, encode_grouped_bbpe

TOKENIZER_PATH = "checkpoints/transformer_attention_relsimple_tokenizer.json"
TOKENIZER_META_PATH = "checkpoints/transformer_attention_relsimple_tokenizer_meta.json"
HPARAMS_PATH = "checkpoints/transformer_attention_relsimple_hparams.json"
...
def train_grouped_transformer_attention_relsimple(train_df: pd.DataFrame, seed=40, max_len=128):
    device = get_device()
    ...
    print("------------------- Train Model Transformer Attention RelSimple ----------------")
    ...
    model = BertRelSimpleTransformer(vocab_size=vocab_size, num_classes=1, pad_idx=pad_id, sep_idx=sep_id, model_dim=model_dim, num_heads=num_heads, num_layers=num_layers, ff_mult=ff_mult, dropout=dropout, max_len=max_len_pe, pooling=pooling).to(device)
    ...
    for epoch in range(1, epochs + 1):
        ...
        if val_cat_acc > best_acc + 1e-6:
            ...
            torch.save(model.state_dict(), "checkpoints/transformer_attention_relsimple_best_acc.pt")
            improved = True
        if val_loss < best_loss - 1e-6:
            best_loss, best_epoch_loss = val_loss, epoch
            torch.save(model.state_dict(), "checkpoints/transformer_attention_relsimple_best_loss.pt")
            improved = True
        torch.save(model.state_dict(), "checkpoints/transformer_attention_relsimple.pt")
        if improved:
            bad = 0
        else:
            bad += 1
...
if __name__ == "__main__":
    ...
    train_grouped_transformer_attention_relsimple(train_data, seed=40, max_len=128)
