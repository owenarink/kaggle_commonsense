...
class LinearModule(object):
    ...
    def __init__(self, in_features, out_features, input_layer=False):
        ...
        N = np.(in_features, out_features)
        weights = N * np.sqrt(2.0 / in_features)
        biases = np.zeros(out_features, dtype=float)
        self.params['weight'] = weights
        self.params['bias'] = biases
        self.grads['weight'] = np.zeros_like(in_features)
        self.grads['bias'] = np.zeros_like(in_features)
        self.cache = None
    ...
    def forward(self, x):
        ...
        """
        self.x = x
        out = x @ self.params['weights'].T + self.biases # y = x * W.T + b
        return out 
