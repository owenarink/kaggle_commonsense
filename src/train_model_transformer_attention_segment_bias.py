...
TOKENIZER_PATH = "checkpoints/transformer_attention_segment_bias_tokenizer.json"
TOKENIZER_META_PATH = "checkpoints/transformer_attention_segment_bias_tokenizer_meta.json"
HPARAMS_PATH = "checkpoints/transformer_attention_segment_bias_hparams.json"
...
def train_grouped_transformer_attention_segment_bias(train_df: pd.DataFrame, seed=40, max_len=128):
    device = get_device()
    ...
    print("------------------- Train Model Transformer Attention Segment Bias ----------------")
    ...
    model = BertSegmentBiasTransformer(
    ...
    for epoch in range(1, epochs + 1):
        ...
        if val_cat_acc > best_acc + 1e-6:
            ...
            torch.save(model.state_dict(), "checkpoints/transformer_attention_segment_bias_best_acc.pt")
            improved = True
        if val_loss < best_loss - 1e-6:
            best_loss = val_loss
            best_epoch_loss = epoch
            torch.save(model.state_dict(), "checkpoints/transformer_attention_segment_bias_best_loss.pt")
            improved = True

        torch.save(model.state_dict(), "checkpoints/transformer_attention_segment_bias.pt")
...
if __name__ == "__main__":
    ...
    train_grouped_transformer_attention_segment_bias(train_data, seed=40, max_len=128)
