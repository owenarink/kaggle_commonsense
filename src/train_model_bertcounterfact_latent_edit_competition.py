...
TOKENIZER_PATH = "checkpoints/transformer_bertcounter_LEC_tokenizer.json"
TOKENIZER_META_PATH = "checkpoints/transformer_bertcounter_LEC_tokenizer_meta.json"
HPARAMS_PATH = "checkpoints/transformer_bertcounter_LEC_hparams.json"
...
def train_grouped_transformer_bbpe(train_df: pd.DataFrame, seed=40, max_len=128):
    ...
    eos_id = tok_info["eos_id"]
    vocab_size = tok_info["vocab_size"]

    print("------------------- TRAIN BertCounterFact Latent Edit Competition Model ----------------")
    ...
    hparams = {
        ...
        "edit_minimality_weight": float(edit_minimality_weight),
    ...
    for epoch in range(1, epochs + 1):
        ...
        for batch_x, batch_y in pbar:
            ...
            batch_x = token_dropout(
                batch_x,
                pad_id=pad_id,
                unk_id=unk_id,
                p=0.10,
                protected_ids=(cls_id, sep_id, eos_id),
            )
            B = batch_x.size(0)
            scores = model(batch_x)   # [B, K] 
        ...
        if val_cat_acc > best_acc + 1e-6:
            ...
            torch.save(model.state_dict(), "checkpoints/transformer_bertcounter_LEC_best_acc.pt")
            improved = True
        ...
        if val_loss < best_loss - 1e-4:
            ...
            torch.save(model.state_dict(), "checkpoints/transformer_bertcounter_LEC_best_loss.pt")
            improved = True

        # also keep a “main” path pointing to best acc
        if improved and val_cat_acc >= best_acc - 1e-9:
            torch.save(model.state_dict(), "checkpoints/transformer_bertcounter_LEC.pt")
