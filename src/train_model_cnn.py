...
def load_prep_data_pairwise(seed=40, max_len=128, min_freq=2, max_vocab=50000):
    ...
    print('------------------- TRAIN PAIRWISE TEXT-RESNET CNN MODEL ----------------')
...
def train_pairwise_cnn(X_tr, X_va, y_tr, y_va, va_group_ids, va_opts, va_gold, vocab):
    ...
    for epoch in range(epochs):
        ...
        print(f"[PAIR-CNN] epoch [{epoch+1}/{epochs}] loss={loss.item():.4f} val_pair_acc={val_pair_acc:.4f} val_cat_acc={val_cat_acc:.4f}")

    total_pbar.close()

    os.makedirs("checkpoints", exist_ok=True)
    torch.save(model.state_dict(), "checkpoints/cnn_pairwise.pt")
    print("Saved pairwise CNN model to checkpoints/cnn_pairwise.pt")

if __name__ == "__main__":
    X_tr, X_va, y_tr, y_va, va_group_ids, va_opts, va_gold, vocab = load_prep_data_pairwise(seed=40)
    train_pairwise_cnn(X_tr, X_va, y_tr, y_va, va_group_ids, va_opts, va_gold, vocab)
    print('train_model_cnn (pairwise) completed\n')
    print('------------------------------------------------')
