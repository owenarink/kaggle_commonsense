...
TOKENIZER_PATH = "checkpoints/transformer_bertcounterfact_tokenizer.json"
TOKENIZER_META_PATH = "checkpoints/transformer_bertcounterfact_tokenizer_meta.json"
HPARAMS_PATH = "checkpoints/transformer_bertcounterfact_hparams.json"
...
def train_grouped_transformer_bbpe(train_df: pd.DataFrame, seed=40, max_len=128):
    ...
    print("------------------- Train Model Bert Counter Fact ----------------")
    ...
    model = BertCounterFactTransformer(
    ...
    for epoch in range(1, epochs + 1):
        ...
        if val_cat_acc > best_acc + 1e-6:
            ...
            torch.save(model.state_dict(), "checkpoints/transformer_bertcounterfact_best_acc.pt")
            improved = True
        ...
        if val_loss < best_loss - 1e-4:
            ...
            torch.save(model.state_dict(), "checkpoints/transformer_bertcounterfact_best_loss.pt")
            improved = True

        # also keep a “main” path pointing to best acc
        if improved and val_cat_acc >= best_acc - 1e-9:
            torch.save(model.state_dict(), "checkpoints/transformer_bertcounterfact.pt")
