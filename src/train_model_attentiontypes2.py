...
from src.augment_attentiontypes2 import augment_grouped_bbpe

TOKENIZER_PATH = "checkpoints/transformer_attentiontypes_2_tokenizer.json"
TOKENIZER_META_PATH = "checkpoints/transformer_attentiontypes_2_tokenizer_meta.json"
HPARAMS_PATH = "checkpoints/transformer_attentiontypes_2_hparams.json"
...
def preprocess_transformer_grouped_bbpe(train_df: pd.DataFrame, seed=40, max_len=128, vocab_size=12000, min_freq=2):
    ...
    tok_info = {
        "pad_id": int(pad_id),
        "unk_id": int(unk_id),
        "sep_id": int(meta.get("sep_id", 2)),
        "cls_id": int(meta.get("cls_id", 3)),
        "eos_id": int(meta.get("eos_id", 4)),
        "tokenizer_path": TOKENIZER_PATH,
...
def train_grouped_transformer_bbpe(train_df: pd.DataFrame, seed=40, max_len=128):
    ...
    protected_ids = (tok_info["sep_id"], tok_info["cls_id"], tok_info["eos_id"])
    vocab_size = tok_info["vocab_size"]

    print("------------------- Train Model Transformer Attention Types 2 ----------------")
    ...
    for epoch in range(1, epochs + 1):
        ...
        for batch_x, batch_y in pbar:
            ...
            batch_x = augment_grouped_bbpe(
                batch_x,
                pad_id=pad_id,
                unk_id=unk_id,
                protected_ids=protected_ids,
                token_dropout_p=0.08,
                swap_p=0.03,
                span_mask_p=0.12,
                span_len=2,
            )
        ...
        if val_cat_acc > best_acc + 1e-6:
            ...
            torch.save(model.state_dict(), "checkpoints/transformer_attentiontypes_2_best_acc.pt")
            improved = True

        if val_loss < best_loss - 1e-4:
            best_loss = val_loss
            best_epoch_loss = epoch
            torch.save(model.state_dict(), "checkpoints/transformer_attentiontypes_2_best_loss.pt")
            improved = True

        if improved and val_cat_acc >= best_acc - 1e-9:
            torch.save(model.state_dict(), "checkpoints/transformer_attentiontypes_2.pt")
...
if __name__ == "__main__":
    ...
    print("attentiontypes_2 completed\n")
    print("------------------------------------------------")
